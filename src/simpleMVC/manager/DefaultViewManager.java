...
import simpleMVC.application.Application;
import simpleMVC.container.AbstractViewContainerFrame;
import simpleMVC.container.ViewContainerFrame;
import simpleMVC.perspective.Perspective;
...
public class DefaultViewManager extends AbstractViewManager
{
	...
	public DefaultViewManager(Application app,Perspective perspective){
		logger.info("DefaultViewManager_1");		
	...
	public ViewContainerFrame getRootView() {
		logger.info("get_root_view_is_started");
		if (rootView == null){
			logger.info("root_view_is_null");
			rootView = new AbstractViewContainerFrame(){
				//Just for instancing it
			};
			logger.info("root_view_is_not_null");
			rootView.setApplication(this.getApplication());
		}else {
			logger.info("root_view_is_not_null");
		}
		logger.info("get_root_view_is_finished");
		return rootView;
	}
